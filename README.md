...
deep nesting and keep controller classes clean.

### Data Storage:
Per the project constraints, no external SQL database is used. Data is managed in-memory using HashMap 
and ArrayList data structures within a MockDatabase class. All data is stored inside concurrent hashmaps to ensure thread safety.
 A GenericDAO (Data Access Object) pattern is implemented to provide a clean, standardized interface for interacting with this mock data.

### Error Handling:
The API features advanced exception mapping to ensure it never leaks internal Java stack traces. Custom
 exceptions (RoomNotEmptyException, LinkedResourceNotFoundException, SensorUnavailableException) are mapped
